<?php
    ...
    private function __construct() {
		...
		try {
			$this->db = new PDO("mysql:host=$servername;dbname=gem_db", $username, $password);
	...
	public function select_from_ord2($table,$id,$ord){
		try {
	...
	public function select_from_team(){
		
		try {
			$stmt = $this->db->prepare("SELECT * FROM team Order BY team_id ASC LIMIT 4");
			$stmt->execute();
			return $stmt;
			$stmt = null;
		} catch (PDOException $e) {
	...
	public function select_from_kitchen_live($id){
		try {
			...
			$que= $this->db->prepare("SELECT * FROM buy_detail a
										left join item b on a.item_id = b.item_id
										WHERE a.handled = ? and b.item_type_id = ? and a.buy_detail_id > ? ORDER BY buy_detail_id ASC");
			$que->execute([$hand,$type,$id]);
			return $que;
			$que = null;			
		} catch (PDOException $e) {
			// For handling error
